...
for block in list_blocks:
    ...
    plt.title("Matrice delle correlazioni (Kendall): {}".format(block), fontsize='xx-large')
    mask = np.triu(np.ones_like(corr_matrix, dtype=bool))
    sns.heatmap(corr_matrix, annot=True, annot_kws={"size": 8}, vmax=1, vmin=-1, center=0, cmap='vlag', mask=mask)
    plt.savefig('Figures/correlation_matrices/correlation_matrices_meter_vs_meter/correlation_matrix_{}.png'.format(block), dpi=300)
