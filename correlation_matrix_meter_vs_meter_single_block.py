...
os.makedirs("Figures/correlation_matrices/correlation_matrices_meter_vs_meter", exist_ok=True)
...
for meter in list_meters:
    ...
    dict_corr.update({meter: df_locale_lettura_differenziale})
...
plt.title("Matrice delle correlazioni (Kendall): {}".format(nome_isola), fontsize='xx-large')
mask = np.triu(np.ones_like(corr_matrix, dtype=bool))
sns.heatmap(corr_matrix, annot=True, annot_kws={"size": 8}, vmax=1, vmin=-1, center=0, cmap='vlag', mask=mask)
plt.savefig('Figures/correlation_matrices/correlation_matrices_meter_vs_meter/correlation_matrix_{}.png'.format(nome_isola), dpi=300)
plt.show()