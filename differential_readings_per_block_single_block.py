...
os.makedirs("Figures/differential_readings_per_block", exist_ok=True)
...
while True:
    try:
        nome_isola = str(input("Insert the block name: "))
        if nome_isola in df['nome_isola'].values:
            break
        print("The block name is not valid.")
    except Exception as e:
        print(e)


# Statistical analysis
df_isola = df[((df['nome_isola'] == nome_isola))]
...
for i in range(len(isola_lettura_differenziale)):
    if isola_lettura_differenziale[i] > limit:
...
plt.plot(dates, isola_lettura_differenziale, label='consumo giornaliero')
...
plt.title("Consumi giornalieri {}".format(nome_isola), fontsize='xx-large')
plt.xlabel('data', fontsize='xx-large')
plt.ylabel('consumo giornaliero', fontsize='large')
plt.gcf().autofmt_xdate()
plt.grid()
plt.legend(fontsize='large')
plt.savefig('Figures/differential_readings_per_block/{}.png'.format(nome_isola), dpi=300)
plt.show()