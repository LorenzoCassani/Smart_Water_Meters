...
plt.savefig('Figures/differential_readings_per_meter/{}.png'.format(nome_locale), dpi=300)
plt.show()
# Plot letture cumulative
'''
plt.figure(figsize = (16, 9))
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%d/%m/%Y'))
plt.gca().xaxis.set_major_locator(mdates.MonthLocator(interval=1))
plt.plot(df_locale_data, df_locale_lettura_cumulativa)
plt.title("letture {}".format(nome_locale), fontsize='xx-large')
plt.xlabel('data', fontsize='xx-large')
plt.ylabel('lettura', fontsize='xx-large')
plt.gcf().autofmt_xdate()
plt.grid()
plt.savefig('../Desktop/Work/Figures/{}_cumulativa.png'.format(nome_locale), dpi=300)
plt.show()
'''